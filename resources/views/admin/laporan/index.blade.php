...
<div class="stat-grid" style="grid-template-columns:repeat(4,1fr); margin-bottom:28px;">
    <div class="stat-card indigo"><div class="stat-icon indigo"><i class="bi bi-hammer"></i></div><div class="stat-label">Total Lelang</div><div class="stat-value">{{ $stats['total_lelang'] }}</div></div>
    <div class="stat-card green"><div class="stat-icon green"><i class="bi bi-check-circle-fill"></i></div><div class="stat-label">Lelang Selesai</div><div class="stat-value">{{ $stats['lelang_selesai'] }}</div></div>
    <div class="stat-card blue"><div class="stat-icon blue"><i class="bi bi-box-seam"></i></div><div class="stat-label">Total Barang</div><div class="stat-value">{{ $stats['total_barang'] }}</div></div>
    <div class="stat-card amber"><div class="stat-icon amber"><i class="bi bi-cash-stack"></i></div><div class="stat-label">Total Transaksi</div><div class="stat-value" style="font-size:18px;">Rp {{ number_format($stats['total_transaksi'],0,',','.') }}</div></div>
</div>
...
<form method="GET" style="background:white; border-radius:12px; border:1px solid var(--border); padding:20px; margin-bottom:20px; display:flex; gap:12px; align-items:flex-end; flex-wrap:wrap;">
    ...
    <a href="{{ route('admin.laporan.export', request()->query()) }}" style="padding:8px 16px; background:#ECFDF5; color:#059669; border-radius:8px; font-size:13px; font-weight:700; text-decoration:none;"><i class="bi bi-download"></i> Export PDF</a>
</form>
    ...
    <table style="width:100%; border-collapse:collapse; font-size:13px;">
        ...
        <tbody>
            ...
            <tr style="border-top:1px solid var(--border);">
                ...
                <td style="padding:12px 16px; font-weight:800; color:var(--brand);">{{ $item->harga_akhir ? 'Rp '.number_format($item->harga_akhir,0,',','.') : '—' }}</td>
                <td style="padding:12px 16px; color:var(--text-muted);">{{ $item->updated_at?->format('d/m/Y H:i') }}</td>
            </tr>
            @empty
            <tr><td colspan="5" style="padding:40px; text-align:center; color:var(--text-muted);">Belum ada data.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
