...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
    ...
    <a href="{{ route('admin.petugas.create') }}" style="padding:8px 18px; background:var(--brand); color:white; border-radius:8px; font-size:13px; font-weight:700; text-decoration:none;">+ Tambah Petugas</a>
</div>
    ...
    <table style="width:100%; border-collapse:collapse; font-size:13px;">
        ...
        <tbody>
            ...
            <tr style="border-top:1px solid var(--border);">
                <td style="padding:12px 16px; font-weight:700;">{{ $p->nama_petugas }}</td>
                ...
                <td style="padding:12px 16px;">
                    <div style="display:flex; gap:6px;">
                        <a href="{{ route('admin.petugas.edit', $p->id_petugas) }}" style="padding:5px 10px; background:#EEF2FF; color:var(--brand); border-radius:6px; font-size:12px; font-weight:700; text-decoration:none;">Edit</a>
                        @if(!$p->isAdmin())
                        <form method="POST" action="{{ route('admin.petugas.destroy', $p->id_petugas) }}" onsubmit="return confirm('Hapus petugas ini?')">
                            @csrf @method('DELETE')
            ...
            @empty
            <tr><td colspan="4" style="padding:40px; text-align:center; color:var(--text-muted);">Belum ada petugas.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
