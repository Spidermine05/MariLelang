...
<div style="display:grid; grid-template-columns:300px 1fr; gap:20px;">
    <div style="background:white; border-radius:12px; border:1px solid var(--border); padding:24px; height:fit-content;">
        ...
        <span style="background:#ECFDF5; color:#059669; padding:4px 12px; border-radius:50px; font-size:12px; font-weight:700;"><i class="bi bi-check-circle-fill"></i> Aktif</span>
        @else
        <span style="background:#FEF2F2; color:#DC2626; padding:4px 12px; border-radius:50px; font-size:12px; font-weight:700;"><i class="bi bi-x-circle-fill"></i> Nonaktif</span>
        @endif
        <div style="margin-top:20px; display:flex; flex-direction:column; gap:8px; font-size:13px;">
            <div><span style="color:var(--text-muted);">Email:</span> {{ $user->email }}</div>
            <div><span style="color:var(--text-muted);">Telepon:</span> {{ $user->telp ?? '—' }}</div>
            <div><span style="color:var(--text-muted);">Alamat:</span> {{ $user->alamat ?? '—' }}</div>
            <div><span style="color:var(--text-muted);">Bergabung:</span> {{ $user->created_at?->format('d/m/Y') }}</div>
        </div>
    </div>

    <div style="background:white; border-radius:12px; border:1px solid var(--border); overflow:hidden;">
        <div style="padding:16px 20px; border-bottom:1px solid var(--border); font-size:14px; font-weight:800;">Riwayat Penawaran</div>
